...
pub mod module {
	...
	impl<T: Config> Pallet<T> {
		...
		/// 创建资产
		#[pallet::weight(10000)]
		pub fn create_asset(origin: OriginFor<T>, currency_id: CurrencyIdOf<T>, amount: BalanceOf<T>) -> DispatchResultWithPostInfo {
			let user = ensure_signed(origin)?;
			/// 这个资产一定是未存在的
			ensure!(!Self::is_exists_metadata(currency_id) && T::MultiCurrency::total_issuance(currency_id) == BalanceOf::<T>::from(0u32), Error::<T>::AssetAlreadyExists);
			T::MultiCurrency::deposit(currency_id, &user, amount)?;
			DicoAssetsInfo::<T>::insert(currency_id, DicoAssetInfo{
				owner: user.clone(),
				metadata: None,
			});
		...
		/// 设置资产信息
		#[pallet::weight(10000)]
		pub fn set_metadata(origin: OriginFor<T>, currency_id: CurrencyIdOf<T>, metadata: DicoAssetMetadata) -> DispatchResultWithPostInfo {
			let user = ensure_signed(origin)?;

			/// metadata要符合基本要求
			ensure!(metadata.name.len() > 2 && metadata.symbol.len() > 1 && metadata.decimals > 0u8 && metadata.decimals < 19, Error::<T>::MetadataErr);
			...
			if asset_info.metadata.is_some() {
				...
				ensure!(asset_info.clone().metadata.unwrap() != metadata, Error::<T>::MetadataNotChange);
				ensure!(asset_info.metadata.clone().unwrap().decimals == metadata.decimals, Error::<T>::ShouldNotChangeDecimals);
			}
		...
		/// 销毁币
		#[pallet::weight(10000)]
		pub fn burn(origin: OriginFor<T>, currency_id: CurrencyIdOf<T>, amount: BalanceOf<T>) -> DispatchResultWithPostInfo {
			let user = ensure_signed(origin)?;
