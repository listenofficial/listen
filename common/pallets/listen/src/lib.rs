...
decl_storage! {
	trait Store for Module<T: Config> as ListenModule {
		...
		/// 解散的群的信息（用于解散后奖励) (session_id, room_id => 房间奖励信息) (已经解散的群)
		...
		/// 上一次处理解散的过期群的周期（记录处理到哪个session了)
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn pay_out(origin) -> DispatchResult {
			...
			if amount.is_zero() {
				return Err(Error::<T>::HaveNoReward)?;
			}
			else {
...
impl <T: Config> Module <T> {
	...
	fn join_do(invitee: T::AccountId, group_id: u64, inviter: T::AccountId, payment_type: Option<InvitePaymentType>) -> DispatchResult{
		...
		Self::add_listener_info(invitee.clone(), group_id.clone());
	...
	fn is_vote_end(room_info: GroupInfo<T::AccountId, BalanceOf<T>,
		...
		// 如果有票总金额先到一半 就结束
		if approve_total_amount * <BalanceOf<T>>::from(2u32) >= consume_total_amount || reject_total_amount * <BalanceOf<T>>::from(2u32) >= consume_total_amount {
	...
	fn is_voting(room: GroupInfo<T::AccountId, BalanceOf<T>, AllProps, Audio, T::BlockNumber, GroupMaxMembers, DisbandVote<BTreeSet<T::AccountId>, BalanceOf<T>>, T::Moment>)
		-> bool
	{
		if !room.disband_vote_end_block.clone().is_zero() && (Self::now().saturating_sub(room.disband_vote_end_block.clone()).is_zero()) {
			return true;
		}
		false
	}
		...
		if vote_result.0 == End {
			...
			if vote_result.1 == Pass{
				...
				if vote_result.2 == <BalanceOf<T>>::from(0u32) {
					...
					// 如果不是直接解散 那么就更新解散投票结束的时间(这个是提前发现的 所以disband_vote_end_block要做更改)
					room.disband_vote_end_block = now;

					<AllRoom<T>>::insert(group_id, room);

					<DisbandingRooms<T>>::mutate(|h| h.insert(group_id, now + T::DisbandDelayTime::get()));
				}
	...
	fn remove_expire_disband_info() {
		...
		/// 如果获取上次过期群的队列长度是0 那么说明上个session已经处理完 接着处理下一个session
		if info.len() == 0 {
