...
use sp_runtime::traits::{Saturating, AtLeast32BitUnsigned, Member, MaybeSerializeDeserialize, SaturatedConversion};
use listen_traits::{ListenHandler, CollectiveHandler};
use ico::{self, ico_trait::IcoHandler};
use frame_support::{
...
pub trait Config: frame_system::Config + ico::Config{
	...
	type Origin: From<IcoRawOrigin<Self::AccountId, MultiBalanceOf<Self>>> + Into<Result<IcoRawOrigin<Self::AccountId, MultiBalanceOf<Self>>, <Self as Config>::Origin>>;
	...
	type IcoHandler: IcoHandler<CurrencyIdOf<Self>, MultiBalanceOf<Self>, Self::AccountId, DispatchError, Self::BlockNumber>;
}
...
// /// Origin for the collective module.
// pub type Origin<T, I=DefaultInstance> = IcoRawOrigin<<T as frame_system::Config>::AccountId, I>;
...
pub struct IcoCollectiveVotes<AccountId, BlockNumber, MulBalance> {
	...
	/// The number of approval IcoCollectiveVotes that are needed to pass the motion.
...
decl_event! {
		...
		Balance = MultiBalanceOf<T>,
	{
		...
		Proposed(AccountId, ProposalIndex, Hash, Balance),
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: <T as frame_system::Config>::Origin {
		...
		fn propose(origin,
			...
			// 如果提议的那个人直接超过阀值 那么直接通过
			if threshold <= user_ico_amount {
				let ico_total_amount = T::IcoHandler::get_total_ico_amount(currency_id, ico_index)?;
		...
		fn close(origin,
			...
			let seats = T::IcoHandler::get_total_ico_amount(currency_id, ico_index)?;
...
> EnsureOrigin<O> for EnsureProportionAtLeast<T, N, D, AccountId> {
	...
	fn try_origin(o: O) -> Result<Self::Success, O> {
		o.into().and_then(|o| match o {
			IcoRawOrigin::Members(n, m) if n * D::VALUE.saturated_into::<MultiBalanceOf<T>>() >= N::VALUE.saturated_into::<MultiBalanceOf<T>>() * m => Ok(()),
			r => Err(O::from(r)),
		})
	}
