...
// NOTICE
// This module is changed from the collective module in Substrate.
// You can find the collective at
//
...
impl<T: Config<I>, I: Instance> Module<T, I> {
	...
	fn normal_close(voting: RoomCollectiveVotes<T::AccountId, T::BlockNumber>, room_id: RoomIndex, proposal_hash: T::Hash) -> DispatchResult {
		...
		if approved {
			let proposal = ProposalOf::<T, I>::get(room_id, proposal_hash).ok_or(Error::<T, I>::ProposalMissing)?;
...
impl<O: Into<Result<RoomRawOrigin<<T as frame_system::Config>::AccountId, I>, O>> + From<RoomRawOrigin<<T as frame_system::Config>::AccountId, I>>,
	AccountId,
	T: Config<I>,
	I: Instance,
...
> EnsureOrigin<O> for EnsureRoomRoot<T, AccountId, I> {
	...
	fn try_origin(o: O) -> Result<Self::Success, O> {
		o.into().and_then(|o| match o {
			RoomRawOrigin::Member(room_id, who) if T::ListenHandler::get_root(room_id).is_ok() && T::ListenHandler::get_root(room_id).unwrap() == who => Ok(()),
			r => Err(O::from(r)),
		})
	}
